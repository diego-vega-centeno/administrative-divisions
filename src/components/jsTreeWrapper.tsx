...
import $ from "jquery";
// jsTree plugin expects jQuery to be available globally
window.jQuery = $;
window.$ = $;
import "jstree";
...
import { JstreeNode } from "../types";

const fetchCountryChildren = async (countrId: string) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_URL}/country/${countrId}/children`,
  );
  if (!response.ok) throw new Error(`Failed fetch of country: ${countrId}`);
  return response.json();
};

const JsTreeWrapper = forwardRef(
  ({ onSelect }: { onSelect: (_: any) => void }, ref) => {
    ...
    useEffect(() => {
      $(treeRef.current!).jstree({
        core: {
          ...
          data: async function (node: JstreeNode, callback: (_: any) => void) {
            try {
              if (node.id == "#") {
        ...
        contextmenu: {
          select_node: false,
          items: function (node: JstreeNode) {
            return {
              select: {
                label: "Select",
                submenu: {
                  childs: {
                    ...
                    action: function (obj: any) {
                      ...
                      tree.open_node(node, function () {
                        ...
                        onSelect(
                          updatedNode.children.map((id: string) =>
                            tree.get_node(id),
                          ),
                        );
                      });

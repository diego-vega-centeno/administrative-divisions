...
const DataTable = memo(
  ({
    ...
    const [rows, setRows] = useState<ComputedDataRelsType>([]);
    const [order, setOrder] = useState<"asc" | "desc">("asc");
    const [orderBy, setOrderBy] = useState<keyof ComputedDataRelType>("name");

    useEffect(() => {
      if (!computedDataRels.length) return;
      setRows(
        [...computedDataRels].sort((a, b) => compare(a, b, order, orderBy)),
      );
    ...
    function compare(
      a: ComputedDataRelType,
      b: ComputedDataRelType,
      order: "asc" | "desc",
      orderBy: keyof ComputedDataRelType,
    ) {
      const av = a[orderBy];
      const bv = b[orderBy];
    ...
    function handleRequestSort(name: keyof ComputedDataRelType) {
                ...
                {headerCells.map((header) => (
                  ...
                  >
                    <TableSortLabel
                      ...
                      onClick={() =>
                        handleRequestSort(
                          header.id as keyof ComputedDataRelType,
                        )
                      }
                    >
                      {header.label}
              ...
              {rows.map((rel) => (
                <TableRow key={rel.id}>
                  <TableCell align="center" sx={tableCell}>
                    {formatValue(rel.admin_level)}
                  </TableCell>
                  <TableCell align="center" sx={tableCell}>
                    {rel.name}
                  </TableCell>
                  <TableCell align="center" sx={tableCell}>
                    {formatValue(rel.area)}
                  </TableCell>
                  <TableCell align="center" sx={tableCell}>
                    {formatValue(rel.perimeter)}
                  </TableCell>
                  <TableCell align="center" sx={tableCell}>
                    {formatValue(rel.population)}
                  </TableCell>
                  <TableCell align="center" sx={tableCell}>
                    {formatValue(rel.popDensity)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>

import {
  ...
  subHeaderCell,
  headerCellContent,
  headerCellToolsContainer,
  headerCellToolsButton,
...
import { RelationsType, RelationsUserLayersType } from "../types/index.ts";

interface FavoritesMenuTableProps {
  activeLayer: string;
  layerRels: RelationsUserLayersType[];
  setActiveLayer: Dispatch<SetStateAction<string>>;
  plotLayer: (_: string) => void;
  groupKey: string;
  deleteSelectedLayer: (_: string) => void;
  editMode: boolean;
  setEditMode: Dispatch<SetStateAction<boolean>>;
  confirm: boolean;
  setConfirm: Dispatch<SetStateAction<boolean>>;
  selectedLayerRelsIds: Set<string>;
  setSelectedLayerRelsIds: Dispatch<SetStateAction<Set<string>>>;
  error: string;
...
export default function FavoritesMenuTable({
  ...
  confirm,
  setConfirm,
  selectedLayerRelsIds,
  setSelectedLayerRelsIds,
  error,
  setError,
  handleEditSave,
}: FavoritesMenuTableProps) {
  ...
  const handlePlotLayer = (groupKey: string) => {
    plotLayer(groupKey);
  };

  const toggle = (id: string) => {
    setSelectedLayerRelsIds((prev: Set<string>) => {
  ...
  return (
    <Table sx={table}>
      <TableHead>
        <TableRow>
          <TableCell align="center" sx={headerCell} colSpan={5}>
            <Box sx={headerCellContent}>
              ...
              {confirm && !editMode && activeLayer === layerId ? (
                ...
                >
                  <Box>
                    <Box component={"span"}>
                      {selectedLayerRelsIds.size} relations will be deleted
                    </Box>
                  </Box>
                  <Box>
                    <Button
                      ...
                      variant="contained"
                      onClick={() =>
                        handleEditSave(groupKey, layerId, layerTitle, newTitle)
                      }
                    >
                      save
                    </Button>
        ...
        <TableRow>
          ...
          <TableCell align="center" sx={subHeaderCell}>
            admin level
          </TableCell>
          <TableCell align="center" sx={subHeaderCell}>
            id
              ...
              <TableCell align="center" sx={favoritesMenuCheckboxCell}>
                {editMode && activeLayer === layerId && (
                  ...
                  >
                    <FontAwesomeIcon
                      icon={
                        selectedLayerRelsIds.has(rel.id) ? faTrash : faSquare
                      }
                      style={{
                        color: selectedLayerRelsIds.has(rel.id) ? "red" : "",
                      }}
                    />
                  </Button>
                )}
